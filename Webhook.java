...
//        //슬랙 침범
//        //String slackUrl = "https://hooks.slack.com/services/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";
//        String slcakUrl = System.getenv("SLACK_WEBHOOK_URL");
//        String payload = "{\"text\": \"" + text + "\"}";
//        //마치 브라우저나 유저인척 하는 것.
//        HttpClient client = HttpClient.newHttpClient(); //새롭게 요청할 클라이언트 생성
//        //요청을 만들어 보자 (fetch)
//        HttpRequest request = HttpRequest.newBuilder()
//                //URI.create를 사용해서 String을 변경
//                .uri(URI.create(slcakUrl)) // 어디로? URI -> Uniform Resource Identifier(Link)
//                .header("Content-Type", "application/json")
//                .POST(HttpRequest.BodyPublishers.ofString(payload))
//                .build(); // 핵심 빌더를 빌드해라
//
//        // 네크워크 과정에서 오류가 있을 수 있기에 선제적 예외처리
//        try {
//            HttpResponse<String> response = client.send(request,
//                    HttpResponse.BodyHandlers.ofString());
//            //2 뭔가됨, 4,5 뭔가 잘못됨. 1은 볼 일이 없고 3은 중간과정이라 3을 주지않는다.
//            System.out.println("response.statusCode() = " + response.statusCode());
//            System.out.println("response.body() = " + response.body());
//        } catch (Exception e) {
//            throw new RuntimeException(e);
